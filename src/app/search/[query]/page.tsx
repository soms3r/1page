...
function buildFlatIndex(): FlatEntry[] {
  ...
  for (const entry of seo) {
    ...
    for (const kw of entry.keywords) {
      ...
      if (!seen.has(slug)) {
        seen.add(slug);
        flat.push({ slug, original: kw, seo: entry, workflow });
      }
    }
    for (const q of entry.relatedQueries) {
      ...
      if (!seen.has(slug)) {
        seen.add(slug);
        flat.push({ slug, original: q, seo: entry, workflow });
      }
    }
  }
  return flat;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <div className="text-lg font-bold text-[var(--accent)]">$ search: {original}</div>
        <p className="text-sm text-[var(--muted)]">
          AI workflow for &quot;{original}&quot;
        </p>
      </div>
      ...
      <div className="border border-[var(--border)] p-4 space-y-3">
        ...
        <Link
          ...
          className="block border border-[var(--border)] p-3 hover:border-[var(--accent)]"
        >
          <div className="text-[var(--accent)] font-bold">&gt; {workflow.title}</div>
        ...
        <Link
          href={`/workflows/${workflow.slug}`}
          className="inline-block text-xs border border-[var(--accent)] text-[var(--accent)] px-3 py-1"
        >
          Open Workflow →
        </Link>
      </div>
      ...
      <div className="space-y-2">
        <div className="text-xs text-[var(--muted)]">Browse by:</div>
        <div className="flex flex-wrap gap-2">
          <Link href={`/category/${workflow.category}`} className="text-xs text-[var(--accent)]">
            {workflow.category}
          </Link>
          {workflow.tags.map((tag) => (
            <Link key={tag} href={`/tag/${tag}`} className="text-xs text-[var(--accent)]">
              #{tag}
            </Link>
          ))}
          ...
          <Link href="/new" className="text-xs text-[var(--accent)]">latest</Link>
        </div>
      </div>
