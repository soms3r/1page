...
const markdownComponents: Components = {
  code: ({ className, children, ...props }) => {
    ...
    if (isInline) {
      return (
        <code className="bg-[#1a1a1a] px-1.5 py-0.5 text-[var(--accent)] text-xs rounded" {...props}>
          {children}
        </code>
      );
    }
    return (
      <pre className="bg-[#111] border border-[var(--border)] p-4 overflow-x-auto my-3">
        {match && (
...
export default function WorkflowClient({ workflow, slug, related, expanded }: { workflow: Workflow; slug: string; related: RelatedWorkflows; expanded: ExpandedContent | null }) {
  ...
  const [bookmarked, setBookmarked] = useState(() => {
    if (typeof window !== "undefined") {
      return !!localStorage.getItem(`bm:${slug}`);
    }
    return false;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-sm text-[var(--accent)] font-bold">$ open {slug}</div>

      <div>
        <h1 className="text-lg font-bold">{workflow.title}</h1>
        <p className="text-sm text-[var(--muted)] mt-1">{workflow.description}</p>
      </div>

      <div className="flex flex-wrap gap-2 text-xs">
        <Link
          href={`/search?category=${workflow.category}`}
          className="px-2 py-0.5 border border-[var(--border)] hover:border-[var(--accent)]"
        >
          {workflow.category}
        </Link>
      ...
      {workflow.body && (
        <ShareUnlock slug={slug} title={workflow.title} locked={workflow.locked}>
          <div className="border border-[var(--border)] p-4">
            <div className="text-xs text-[var(--muted)] mb-3">Template:</div>
      ...
      {hasVariables && (
        <div className="space-y-3 border border-[var(--border)] p-4" onKeyDown={handleKeyDown}>
          <div className="text-sm font-bold">$ variables</div>
      ...
      {!hasVariables && (
        <div className="flex gap-2">
          <button onClick={generate}>Copy Prompt</button>
        </div>
      )}

      {showPreview && output && (
        <div ref={outputRef} className="space-y-2 border border-[var(--accent)] p-4">
      ...
      {expanded && (
        <div className="space-y-4 pt-4 border-t border-[var(--border)]">
          <div className="text-xs text-[var(--muted)]">Workflow Guide</div>
      ...
      {(related.sameCategory.length > 0 || related.trending.length > 0 || related.tagMatched.length > 0) && (
        <div className="space-y-4 pt-4 border-t border-[var(--border)]">
          <div className="text-xs text-[var(--muted)]">Related Workflows</div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {related.sameCategory.length > 0 && (
              <RelatedList title="Same Category" items={related.sameCategory} />
            )}
            {related.trending.length > 0 && (
              <RelatedList title="Trending" items={related.trending} />
            )}
            {related.tagMatched.length > 0 && (
              <RelatedList title="Similar Tags" items={related.tagMatched} />
            )}
          </div>
        </div>
      )}
    ...
    <div className="space-y-1">
      ...
      {items.map((w) => (
        <Link
          ...
          className="block border border-[var(--border)] p-2 hover:border-[var(--accent)] text-xs"
        >
          <div className="text-[var(--accent)] truncate">{w.title}</div>
          {w.description && (
            <div className="text-[var(--muted)] truncate mt-0.5">{w.description}</div>
          )}
        </Link>
      ))}
    </div>
