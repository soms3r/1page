...
export function generateMetadata(): Metadata {
  ...
  const desc = "Most popular AI prompt workflows, ranked by featured status, recency, and tag popularity.";
  return {
    title: `Trending Workflows — ${suffix}`,
    description: desc,
    openGraph: { title: `Trending Workflows — ${suffix}`, type: "website" },
  };
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-[var(--muted)] mt-1">Top {workflows.length} workflows by rank</p>
      </div>
      ...
      <div className="divide-y divide-[var(--border)] border border-[var(--border)] rounded-lg bg-[var(--surface)]">
        {workflows.map((w, i) => (
          <Link key={w.slug} href={`/workflows/${w.slug}`}
            ...
            <span className="text-[var(--muted)] text-xs w-5">#{i + 1}</span>
